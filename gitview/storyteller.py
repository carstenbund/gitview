...
class StoryTeller:
    ...
    def __init__(self, backend: Optional[str] = None, model: Optional[str] = None,
                 ...
                 max_phases_per_prompt: int = 40, storylines: Optional[List[Dict[str, Any]]] = None,
        ...
        # Sections rendered deterministically (e.g. by gitview.evidence); a key
        # present here is written as-is instead of being generated by the model.
        self.precomputed_sections = dict(precomputed_sections or {})
    ...
    def generate_global_story(self, phases: List[Phase],
        ...
        # Generate different story sections (pre-rendered ones need no model call)
        generators = [
            ('executive_summary', 'Executive summary', self._generate_executive_summary),
            ('timeline', 'Timeline', self._generate_timeline),
            ('technical_evolution', 'Technical evolution', self._generate_technical_evolution),
            ('deletion_story', 'Deletion story', self._generate_deletion_story),
            ('full_narrative', 'Full narrative', self._generate_full_narrative),
        ]
        stories = {}
        for key, label, generate in generators:
            if key in self.precomputed_sections:
                print(f"  - {label}... (from evidence, no LLM call)")
                stories[key] = self.precomputed_sections[key]
            else:
                print(f"  - {label}...")
                stories[key] = generate(phase_summaries, repo_name)
        for key, text in self.precomputed_sections.items():
            stories.setdefault(key, text)
    ...
    def _generate_section_with_batches(
        ...
        Batches are stitched in chronological order without a merge call:
        a merge prompt sees only prose, no facts, and is where invented
        ...
        for index, batch in enumerate(batches, 1):
            ...
            note = (f"This is part {index} of {len(batches)} of the {section_name}: it covers phases "
                    f"{batch[0]['phase_number']}-{batch[-1]['phase_number']} ({span}) only. Write about "
                    f"these phases only; other parts cover the rest, so add no overall introduction or "
                    f"conclusion and do not speculate about what came before or after.")
            prompt = self._with_facts(build_prompt_fn(batch, repo_name), note)
            messages = [LLMMessage(role="user", content=prompt)]
            response = self._generate_with_context_guard(
                messages, initial_max_tokens=initial_max_tokens
            )
    ...
    def _with_facts(self, prompt: str, note: str = '') -> str:
        ...
        extra = '\n\n'.join(x for x in (self.facts, note) if x)
        if not extra:
            return prompt
        head, sep, tail = prompt.rstrip().rpartition('\n')
        if not sep:
            return f"{prompt}\n\n{extra}\n"
        return f"{head}\n\n{extra}\n\n{tail}"

    @staticmethod
    def _stitch(section_name: str, parts: List[tuple]) -> str:
        """Join batch outputs chronologically; timelines already carry per-phase headings."""
        if section_name == 'timeline':
            return '\n\n'.join(text for _, text in parts)
        return '\n\n'.join(f"### {span}\n\n{text}" for span, text in parts)
